...
class Ri_setup_comp_cession_distribution_model extends MY_Model
{
    protected $table_name = 'ri_setup_comp_cession_distribution';
    ...
    protected $fields = ['treaty_id', 'portfolio_id', 'company_id', 'broker_id', 'distribution_percent', 'flag_leader'];
    ...
    /**
     * Save Compulsory Cession Distribution of a Portfolio in a Treaty
     *
     *
     ...
     * @return bool
     */
    public function save_distribution($treaty_id, $portfolio_id, $data, $old_distribution)
    {
        // Extract All Data
            ...
            if( !empty($reinsurer_ids) && count($reinsurer_ids) === count($distribution_percent) )
            {
                for($i=0; $i < count($reinsurer_ids); $i++)
                {
                    ...
                    if( in_array($company_id, $old_reinsurers_ids) )
                    {
                        ...
                        $where = ['treaty_id' => $treaty_id, 'portfolio_id' => $portfolio_id, 'company_id' => $company_id];
    ...
    public function get_distribution_by_treaty_portfolio($treaty_id, $portfolio_id)
    {
    ...
    public function delete_distribution($treaty_id, $portfolio_id, $company_ids)
    {
        $status         = FALSE;
        $old_records    = [];
        if($company_ids)
