...
class Tariff_misc_epa_model extends MY_Model
{
    protected $table_name = 'master_tariff_misc_epa';
    ...
    public function validation_rules($action, $formatted=false)
    {
        if($action == 'add')
        {
            $defaults = [
                [
                    'field' => 'fiscal_yr_id',
                    'label' => 'Fiscal Year',
                    'rules' => 'trim|required|integer|max_length[3]|callback__cb_tariff_misc_epa_check_duplicate',
                    '_type'     => 'dropdown',
        ...
        $rules = [
             ...
             * Structure:
             *  [{
             *      tarifTitle1:tarifRate1,
             *      tarifTitle2: tarifRate2,
             *
             *  }]
             */
            'tariff' => [
                [
                    'field' => 'tariff[bc_above]',
                    '_key'  => 'bc_above',
                    'label' => 'Abobe Base Camp Staff Rate(%)',
                    'rules' => 'trim|required|prep_decimal|decimal|max_length[5]',
                    '_type'     => 'text',
                    '_required' => true
                ],
                [
                    'field' => 'tariff[bc_named]',
                    '_key'  => 'bc_named',
                    'label' => 'Base Camp Nammed Staff Rate(%)',
                    'rules' => 'trim|required|prep_decimal|decimal|max_length[5]',
                    '_type'     => 'text',
                    '_required' => true
                ],
                [
                    'field' => 'tariff[bc_unnamed]',
                    '_key'  => 'bc_unnamed',
                    'label' => 'Base Camp Un-Nammed Staff Rate(%)',
                    'rules' => 'trim|required|prep_decimal|decimal|max_length[5]',
                    '_type'     => 'text',
                    '_required' => true
                ],
                [
                    'field' => 'tariff[medical]',
                    '_key'  => 'medical',
                    'label' => 'Medical Rate(%)',
                    'rules' => 'trim|required|prep_decimal|decimal|max_length[5]',
                    '_type'     => 'text',
    ...
    public function get_index_rows()
    {
         ...
         */
        $list = $this->get_cache('trfmisc_epa_index_list');
        if(!$list)
        {
            $list = $this->db->select('PTMISCBB.id, PTMISCBB.fiscal_yr_id, PTMISCBB.active, FY.code_en, FY.code_np')
                                ...
                                ->get()->result();
            $this->write_cache($list, 'trfmisc_epa_index_list', CACHE_DURATION_DAY);
        }
        return $list;
    }
    ...
    public function get_by_fy_portfolio($fiscal_yr_id, $portfolio_id)
    {
         ...
         * Get Cached Result, If no, cache the query result
         */
        $cache_name = 'trfmisc_epa_bfp_' . $fiscal_yr_id . '_' . $portfolio_id;
    ...
    public function log_activity($id, $action = 'C')
    {
        ...
        // Save Activity Log
        $activity_log = [
            'module' => 'portfolio_tarrif_misc_epa',
            'module_id' => $id,
            'action' => $action
        ];
