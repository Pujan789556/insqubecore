...
if ( ! function_exists('_OBJ_AGR_CROP_pre_save_tasks'))
{
	/**
	 ...
	 * @return array
	 */
	function _OBJ_AGR_CROP_pre_save_tasks( array $data, $record )
	{
		$items = $data['object']['items'];

		$v_rules = _OBJ_AGR_CROP_validation_rules(IQB_SUB_PORTFOLIO_AGR_CROP_ID);
...
if ( ! function_exists('_TXN_AGR_CROP_premium_goodies'))
{
	/**
	 ...
	 * @return	array
	 */
	function _TXN_AGR_CROP_premium_goodies($policy_record, $policy_object)
	{
		$CI =& get_instance();
...
if ( ! function_exists('__save_premium_AGR_CROP'))
{
	/**
	 ...
	 * @return json
	 */
	function __save_premium_AGR_CROP($policy_record, $endorsement_record)
	{
		$CI =& get_instance();
			...
			if($CI->form_validation->run() === TRUE )
        	{
				...
				try{
					...
					foreach($object_attributes->items as $single)
					{
						...
						$tariff = _OBJ_AGR_tariff_by_type(IQB_SUB_PORTFOLIO_AGR_CROP_ID, $single->breed);
