...
if ( ! function_exists('_OBJ_AGR_BEE_tariff_by_type'))
{
	 ...
	 * @return	Object
	 */
	function _OBJ_AGR_BEE_tariff_by_type( $bs_agro_category_id )
	{
		$CI =& get_instance();
		...
		if( !$valid_tariff)
		{
			throw new Exception("Exception [Helper: ph_agr_bee_helper][Method: _OBJ_AGR_BEE_tariff_by_type()]: No Tariff found for supplied Category ({$bs_agro_category_id})");
		}
	...
	function __save_premium_AGR_BEE($policy_record, $endorsement_record)
	{
		...
		if( $CI->input->post() )
		{
			...
			try {
				...
				$tariff = _OBJ_AGR_BEE_tariff_by_type($object_attributes->bs_agro_category_id);
			 ...
			 */
			$validation_rules = _TXN_AGR_BEE_premium_validation_rules($policy_record, $pfs_record, $old_object, TRUE );
