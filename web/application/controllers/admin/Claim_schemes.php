...
class Claim_schemes extends MY_Controller
{
	/**
	 * Controller URL
	 */
	...
	public function edit($id)
	{
		// Valid Record ?
		$id = (int)$id;
		$record = $this->claim_scheme_model->find($id);
		if(!$record)
		{
	...
	private function _save($action, $record = NULL)
	{
		...
		if( $this->input->post() )
		{
			...
			$rules = $this->claim_scheme_model->validation_rules;
	...
	public function delete($id)
	{
		 ...
		 * Safe to Delete?
		 */
		if( !safe_to_delete( 'Claim_scheme_model', $id ) )
		{
			return $this->template->json($data);
		}

		// Admin Constraint?
		$done = $this->claim_scheme_model->delete($record->id);
		if($done)
		{
			$data = [
				'status' 	=> 'success',
    ...
    {
    	$name = strtolower( $name ? $name : $this->input->post('name') );
    	$id   = $id ? (int)$id : (int)$this->input->post('id');
        if( $this->claim_scheme_model->check_duplicate(['LOWER(name)' => $name], $id))
        {
