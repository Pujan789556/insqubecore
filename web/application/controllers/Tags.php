...
class Tags extends MY_Controller
{
	...
	private function _save($action, $record = NULL)
	{
		...
		if( $this->input->post() )
		{
			...
			$rules = $this->tag_model->validation_rules;
			...
			if( $this->form_validation->run() === TRUE )
			{
				...
				if($action === 'add')
				{
					...
					$done = $this->tag_model->insert($data, TRUE); // No Validation on Model
				}
				else
				{
					// Now Update Data
					$done = $this->tag_model->update($record->id, $data, TRUE);
				}
	...
	public function delete($id)
	{
		 ...
		 * Safe to Delete?
		 */
		if( !safe_to_delete( 'Tag_model', $id ) )
		{
			return $this->template->json($data);
		}

		// Admin Constraint?
		$done = $this->tag_model->delete($record->id);
		if($done)
		{
			$data = [
				'status' 	=> 'success',
    ...
    public function check_duplicate($name, $id=NULL){
        ...
        if( $this->tag_model->check_duplicate(['name' => $name], $id))
        {
