    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="box box-solid box-bordered scroll-x">
                <?php
                $tariff_json        = $type == 'm' ? $record->tariff_medical : $record->tariff_package;
                $tariff_records     = json_decode($tariff_json ?? '[]');
                $item_count         = count( $tariff_records);
                ?>
                    ...
                    <tbody>
                        <?php
                            if($item_count):
                                ...
                                <tr <?php echo $i == 0 ? 'id="__tmi_tariff_row"' : '' ?>>
                                    <?php foreach($form_elements as $single_element):?>
                                        <td>
                                             ...
                                             * Load Single Element
                                             */
                                            $single_element['_default']    = $tariff_records[$i]->{$single_element['_key']} ?? '';
                ...
                <div class="box-footer bg-info">
                    <a href="#" class="btn bg-teal" onclick="__duplicate_tr('#__tmi_tariff_row', this)">Add More</a>
                </div>
            </div>
        </div>
    </div>
