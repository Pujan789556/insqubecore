...
if( $record->status === IQB_POLICY_STATUS_VERIFIED )
{
    ...
    // Transaction Status
    if($endorsement_record->status === IQB_ENDORSEMENT_STATUS_VERIFIED || $endorsement_record->status === IQB_ENDORSEMENT_STATUS_RI_APPROVED )
    {
        $status_sentence .= "<br/>Please <strong>Generate Policy Voucher</strong> from <strong>Transactions</strong> Tab.";
    }
    else if( $endorsement_record->status == IQB_ENDORSEMENT_STATUS_VOUCHERED )
    {
        $status_sentence .= "<br/>Please <strong>Generate Policy Invoice</strong> from <strong>Vouchers</strong> Tab.";
    }
    else if( $endorsement_record->status === IQB_ENDORSEMENT_STATUS_INVOICED )
    {
...
{
    echo '<div class="alert alert-danger"><h4><i class="fa fa-warning margin-r-5"></i>Pending RI-Approval.</h4></div>';
}
?>
