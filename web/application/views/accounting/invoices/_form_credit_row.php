...
<?php endif;?>
    <td>
        <div class="form-group">
            ...
            <span class="text-purple _text-ref-account" readonly><?php echo $row ? ac_account_group_path_formatted($row->acg_path, $row->account_name) : '';?></span>
            <input type="hidden" name="account_id[cr][]" value="<?php echo $row ? $row->account_id : '';?>" data-field="account_id">
        </div>
    </td>
    <td>
        <div class="form-group">
            <div class="col-md-4">
                <?php
                $party_type_element['_default'] = $row ? $row->party_type : '';
            ...
            <span class="mrg-r-5 text-purple _text-ref-party" readonly><?php echo $row ? $row->party_name : ''?></span>
            <input type="hidden" name="party_id[cr][]" value="<?php echo $row ? $row->party_id : '';?>" data-field="party_id">
        </div>
    </td>
    <td>&nbsp;</td>
    <td class="text-right amount">
        <?php
        $amount_element['_default'] = $row ? $row->amount : '';
        $this->load->view('templates/_common/_form_components_inline', [
