	...
	private function _save($action, $customer_record, $record = NULL, $from_widget='n')
	{
		...
		if( $this->input->post() )
		{
			...
			if($action === 'add')
			{
				...
				$v_rules = $this->object_model->validation_rules;
			}
			else
			{
			...
			$done = FALSE;
			$object_v_rules = _PO_validation_rules($portfolio_id);
			$v_rules = array_merge($v_rules, $object_v_rules);
			...
			else
			{
				...
				return [
					...
					'message' 		=> $message,
					'reloadForm' 	=> true,
					'form' 			=> $this->load->view('objects/_form',
											[
												'form_elements' 					=> $v_rules,
												'html_form_attribute_components'	=> $this->gaf($portfolio_id, 'html', $attributes),
												'record' 							=> $record,
												'action' 							=> $action
											], TRUE)
				];
